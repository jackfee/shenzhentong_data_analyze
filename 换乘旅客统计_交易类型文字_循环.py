...
for i in range(1,32):
    during_time_begin=datetime.datetime.now()
    print('第',i,'个开始用时:',during_time_begin-start_time)
    ...
    df_tmp['换乘编号']=df_tmp['换乘偏移']+'_'+df_tmp['交易类型']  #换乘编号，用于判断换乘发生的类型，前面的数表示下一次换的交通工具，个位数表示前一次的类型
    ...
    transfer_type=transfer_type[transfer_type['时间变化']<30]      #时间变化小于30用于分析换乘客流
    ...
    print('此样本量可能大于输出csv里面的样本总量，因为时间变化小于90min，且剔除了个人单次出行数据')
    result=transfer_type.groupby(['换乘编号']).count()
    result=result.rename(columns={'时间变化':'样本量'})
    write_path=r'G:\深圳通公交数据\数据分析结果\常旅客换乘结果'
    writefilename=file_name+'换乘旅客分析结果.xlsx'
    write_path_name=write_path+writefilename
    result=result.sort_values(['样本量'],ascending=False)
    result['换乘类别']=result.index
    result.index=range(result.shape[0])
    sample_size={'换乘类别':'样本量','样本量':sample_valid_size}
    result=result.append(sample_size,ignore_index=True)
